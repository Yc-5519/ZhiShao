...
class AppService:
    ...
    def manual(self):
        ...
        return (
            ...
            f"公网看护页：{public_url}\n"
            f"局域网备用入口：{lan_url}\n"
            "页面默认只显示状态卡和脱敏骨架；真实画面默认关闭，开启前必须通过隐私复核。\n\n"
            "一、系统分工\n"
            "RDK X5：负责摄像头采集、BPU 姿态检测、摔倒判断、云台跟随、本地 Web 看护页和飞书消息。\n"
            "云服务器：负责公网入口、访问健康检查和云端 VLM 大脑转发。\n"
            "DashScope/Qwen-VL：负责隐私复核、视觉问答、摔倒复核和日报总结。\n\n"
            "二、常用查看指令\n"
            "说明书：发送这份完整使用说明。\n"
            "帮助 / 指令：发送精简指令菜单。\n"
            "菜单 / 控制面板：发送可点击按钮卡片。\n"
            "实时监控 / 监控链接：返回公网看护页、健康检查和局域网备用入口。\n"
            "系统自检：检查主程序、摄像头、云台串口、公网入口、VLM 大脑和访问记录。\n"
            "查看状态 / 设备状态 / 系统状态：查看运行状态、看护结论、摄像头、云台、模式和今日数据。\n"
            "查看隐私状态：查看真实画面是否开启、剩余时间和最近一次隐私复核原因。\n"
            "最近事件：查看今天最近的告警、隐私复核、家属处置等记录。\n"
            "日报：立即生成并发送今日健康日报。\n\n"
            "三、隐私画面指令\n"
            "我想确认一下 / 临时查看真实画面：申请临时打开真实画面；系统会先调用云端 VLM 大脑做隐私复核，通过后限时开启。\n"
            "关闭真实画面：立即关闭真实画面，恢复默认脱敏看护。\n"
            "说明：复核不通过时会显示具体原因，例如服务不可用、模型不确定或画面存在隐私风险。\n\n"
            "四、人物锁定与云台指令\n"
            "锁定当前人物 / 锁定当前目标：把当前画面中的可信人体作为跟随对象。\n"
            "锁定老人：等同于锁定当前人物，方便家属记忆。\n"
            ...
            "推荐演示顺序：系统自检 -> 监控链接 -> 现在安全吗 -> 临时查看真实画面 -> 日报。\n"
            "系统默认基于脱敏骨架、状态数据和必要的云端 VLM 复核回答，不主动发送真实摄像头画面。"
        )

    def quick_help(self):
        return (
            "智哨指令菜单\n\n"
            "常用：监控链接、系统自检、现在安全吗、他在干什么、日报\n"
            "查看：说明书、查看状态、查看隐私状态、最近事件、当前活动\n"
